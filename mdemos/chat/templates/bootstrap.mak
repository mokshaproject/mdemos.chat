...
var gatherOptions = function(roomOptions, container) {
    ...
    for (var key in roomOptions) {
        opts[key] = roomOptions[key]
    }
    for (var i = 0; i < containerOptions.length; ++i) {
        var key = containerOptions[i]
        var option = container.getAttribute(key)
        if (option) {
            opts[key] = option
        }
    }
    if (!opts.roomId) {
        var roomId = container.getAttribute('roomId')
        opts.roomId = roomId ? roomId : 'default'
    }
    if (!opts.width) {
        opts.width = 350;
    }
...
var loaded = function() {
    ...
    var container = document.getElementById('willowchat');
    if (!container) {
        container = document.createElement('div');
        container.setAttribute('id', 'willowchat');
    }
    ...
    WillowChat.util.optionsCallback = function(result) {
        success = result[0]
        if (!success) {
            throw new Error("Failed: " + result[1])
        }
        room = result[1]
        var opts = gatherOptions(room, container);
        ifr = document.createElement('iframe')
        var staticurl = baseurl + '/toscawidgets/resources/mdemos.chat.chat/static'
       ...
       var chromeCallback = function() {
            ...
            chrome = new WillowChat.chrome.map[opts.theme](container, opts)
        }
